window.addEventListener("DOMContentLoaded", () => {
  ...
  const matcher = (left_tag, right_tag) => {
    let current = edit_area.value.slice(
    ...
    if (current.includes(left_tag) && current.includes(right_tag)) {
      current = current.slice(
        left_tag.length,
        current.length - right_tag.length
      );
    } else {
      current = left_tag + current.substring(0, current.length) + right_tag;
    }
    edit_area.value =
      edit_area.value.slice(0, edit_area.selectionStart) +
      current +
      edit_area.value.slice(edit_area.selectionEnd, edit_area.value.length);
  };
